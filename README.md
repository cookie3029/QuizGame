...
### 2. 퀴즈 주제 선정 이유
컴퓨터 전공생으로서 평소에 하드웨어 분야에 워날 관심과 애정이 많다 보니, 관련 전공 지식을 바탕으로 직접 문제를 고민하고 만들어보면 정말 흥미롭고 재미있는 경험이 될 것 같다는 생각이 들었다. 단순히 지식을 수동적으로 받아들이는 것을 넘어, 하드웨어의 동작 원리와 구조적 특징을 담은 문제를 직접 출제해 보는 과정 그 자체가 큰 즐거움으로 다가왔고, 저처럼 컴퓨터 하드웨어에 관심이 많은 분들과 함께 문제를 나누며 소소한 재미를 느껴보고 싶다는 마음에서 이번 출제를 결심하게 되었다.
